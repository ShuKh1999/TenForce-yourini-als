...
namespace Test_Taste_Console_Application.Domain.Services
{
    ...
    public class ScreenOutputService : IOutputService
    {
        ...
        private readonly PlanetService _planetService1;
        ...
        public ScreenOutputService(PlanetService planetService1) 
        {
            _planetService1 = planetService1;
        }
        ...
        public async Task DisplayPlanetMoonTemperaturesAsync()
        {
            Console.WriteLine("Fetching planets with moons and their moon average temperatures...\n");

            var results = await _planetService1.GetPlanetsWithMoonTempAsync();

            foreach (var item in results)
            {
                var tempDisplay = item.AverageMoonTemperature.HasValue
                    ? $"{item.AverageMoonTemperature:F2} °C"
                    : "N/A";

                Console.WriteLine($"Planet: {item.PlanetName} | Moons: {item.MoonCount} | Avg Moon Temp: {tempDisplay}");
            }
        }
    }
}
